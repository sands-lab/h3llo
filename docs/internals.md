...
- **Unified hostname registration**: The orchestrator sends a single `SetHostnames { hosts: HashSet<String> }` command at startup with all unique peer endpoint hostnames. IP literals are handled by the DNS module directly (immediate `IpResolved` event, max TTL). The DNS module owns hostname tracking, refresh scheduling, and TTL-based expiration internally.
- **Event-driven IP lifecycle**: The orchestrator reacts to two DNS events: `IpResolved` (new IP discovered) triggers TX actor creation; `IpExpired` (IP TTL elapsed or hostname removed) triggers bound removal. This replaces the previous `DnsAnswer`-based approach and eliminates the orchestrator's `dns_ticker` refresh loop.
