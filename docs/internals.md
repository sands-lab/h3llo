...
**Buffer allocation strategy**: Data-plane buffers use `BufFactory::get_max_buf()` + `pop_front(HEADROOM)` with 10 bytes of headroom reserved (9 bytes DGRAM_PREFIX + 1 byte Context ID). This headroom serves dual purposes: (1) zero-copy H3 datagram encoding via `add_prefix(&[CONTEXT_ID_IP])`, and (2) zero-copy TUN TX via `TunBuf`, which prepends a zeroed `virtio_net_hdr` (also 10 bytes) using `add_prefix` without allocation. A compile-time assertion guards the HEADROOM >= VIRTIO_NET_HDR_LEN invariant. H3 datagram decoding uses `pop_front(1)` to strip the Context ID in-place, restoring the headroom for TUN TX. TUN RX receives directly into `TunBuf::alloc_uninit()` (same headroom allocation), then extracts the inner `PooledBuf` via `into_pooled()` — zero-copy from kernel to packet channel. H3 and BareUDP RX use `alloc_packet_buf()` (copy into pooled buffer with headroom). Buffers without headroom (e.g., `BufFactory::buf_from_slice()` in tests) cause `TunBuf` to fall back to alloc + copy.
