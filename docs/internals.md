...
- An actor-wide `HashMap<DnsQuery, PendingQuery>` stores in-flight work. Each timeout tick scans the map once and extracts every expired query directly into the retry set; pending volume is bounded by the global query pacing rate and query timeout.
