...
- An actor-wide `OrderedHashMap<DnsQuery, PendingQuery>` stores in-flight work in send-time order. DNS responses remove matching entries by key without changing the relative order of the remainder; timeout handling pops the expired prefix from the front and stops at the first unexpired query.
