...
Observability summary: interface loops emit cumulative metrics (batches/packets/bytes, drops, and drop-reason breakdowns) on a timer; the orchestrator stores snapshots in a `MetricsStore` and exposes them via `GET /metrics` in OpenMetrics format using the `prometheus-client` crate.
...
- Prometheus exposition: the orchestrator stores the latest cumulative snapshot per unique label set in a `MetricsStore` (`HashMap<TransportLabels, TransportMetrics>` behind `Arc<Mutex<_>>`, defined in `src/api.rs`). On `GET /metrics`, the `prometheus-client` crate's `Collector` trait encodes five counter families (`h3llo_transport_packets`, `h3llo_transport_bytes`, `h3llo_transport_batches`, `h3llo_transport_drops`, `h3llo_transport_drop_bytes`) using `ConstCounter` values — no atomic operations on data-plane hot paths. The `Mutex` is only locked during scrapes (control plane), never contended by datapath actors. Labels: `kind`, `direction`, `peer_id`, plus `outcome` or `reason`. Output uses OpenMetrics text format. Snapshots are kept forever (Prometheus handles staleness natively via its 5-minute staleness marker).
