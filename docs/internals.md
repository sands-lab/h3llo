...
Observability summary: interface loops emit cumulative metrics (batches/packets/bytes, drops, and drop-reason breakdowns) on a timer; the orchestrator stores snapshots in a plain `HashMap`; on scrape, the API actor requests a snapshot clone via event and renders locally using the `prometheus-client` crate. No shared mutable state exists in the metrics path.
