...
2. DNS resolver actor: bind a UDP socket by interface index (Linux/macOS via `bind_device_by_index_v4/6`, Windows via `IP_UNICAST_IF` / `IPV6_UNICAST_IF`) when available. The actor runs a `select` over its command queue, UDP socket, a timer tick at `dns_query_timeout / 2` interval, and a refresh timer (configurable via `tuning.dns_refresh_interval`). The orchestrator sends a single `SetHostnames { hosts: HashSet<String> }` command with all peer endpoint hostnames; the DNS module owns hostname registration and refresh scheduling internally. For each hostname, the resolver assigns random transaction IDs for A and AAAA queries, serialized with a configurable interval (`tuning.dns_query_interval`, default 50ms) between each send to avoid triggering rate limits on public DNS servers. Pending queries are tracked in a per-hostname structure inside `DnsState`, keyed by hostname. Each hostname holds a `pending: HashMap<RecordType, (u16, Instant)>` for in-flight A/AAAA queries and a `next_refresh_at: Instant` for refresh scheduling. Response matching extracts the hostname from the DNS response's question section for O(1) lookup, then validates the transaction ID against the pending entry—providing dual (hostname + txid) validation. `trigger_refresh` skips hostnames whose `next_refresh_at` is still in the future, preventing query floods when `SetHostnames` commands and the refresh timer fire in quick succession. The resolver maintains unified internal state tracking each registered hostname's resolved IPs with TTL-based expiration (floor: `tuning.dns_min_ttl`, default 300 seconds). On state change (new IP, IP expired, hostname added/removed), the resolver sets a dirty flag and arms a debounce timer (`tuning.dns_snapshot_delay`, default 100ms). When the timer fires, the resolver emits a state snapshot (`HashMap<String, HashSet<IpAddr>>`) and clears the dirty flag. The ticker (at `dns_query_timeout / 2` interval) handles only query timeout retries and TTL-based expiration. The orchestrator computes deltas by diffing the new snapshot against the previous one and applies adds/removes accordingly. DNS warnings (NXDOMAIN, recursion refusal) are logged at origin via `warn!` instead of propagating through events. Truncated (TC) responses are treated as packet loss: the pending entry is preserved and retried after `dns_query_timeout` elapses. Timer ticks re-send timed-out entries with new transaction IDs.
...
- The DNS module owns the refresh timer internally; every `tuning.dns_refresh_interval` seconds (when nonzero), it re-queries registered hostnames whose per-entry `next_refresh_at` timestamp has passed. After sending, `next_refresh_at` advances by the refresh interval, throttling redundant queries when `SetHostnames` and the periodic timer overlap.
