...
- **Actor-owned inbox**: `ActorBus` creates one unbounded control-plane inbox when an actor is spawned. The actor exclusively owns its `ActorContext`; callers receive an `ActorRef` that addresses the inbox but cannot access its receiver. Each context also carries an inherited owner address; root mailboxes own themselves, and every descendant spawned from the context retains the same owner. The diagnostic actor name is retained by supervision and included in `ActorExit`. Data-plane packet paths remain explicit bounded `mpsc::channel(PACKET_QUEUE_DEPTH)` queues owned by their consumers.
- **Message passing**: actors send the unified `Event` enum through `ActorContext::send` to an explicit `ActorRef`, or through `ActorContext::notify_owner` when reporting metrics, connection state, DNS state, or API requests to their inherited owner. A send fails when the target actor has exited, so dead recipients are observable. High-volume packet batches bypass the bus and use their dedicated bounded channels.
...
- **Supervision**: `ActorBus` owns the runtimes and drives task monitoring; actors use `ActorContext::spawn` as the only production spawn entry point. Every actor registers a diagnostic name and selects its runtime and `Critical`, `Restartable`, or `Detached` policy; the context automatically associates the actor with its inherited owner, so supervision is independent of both call-site plumbing and the returned error. `ActorBus` delivers `Event::ActorExited` to that owner. The Orchestrator owns current production actors and applies process failure, peer reconciliation, or logging respectively. Actors return `ActorExitResult`:
...
2. **`spawn_*`** - Spawns the actor task:
   - Takes prepared actor or actor-group state plus the parent `ActorContext` and channel capacities
   - Creates bounded data channels internally where needed
   - Spawns through `ActorContext::spawn` with a diagnostic name, an explicit runtime and supervision policy; the actor inherits its owner's address from the parent context
   - Returns an `ActorRef`, the data-plane senders needed by callers, or both; lifecycle task handles remain internal to `ActorBus`
   - Example: `spawn_bare_rx(group, packet_queue_depth, ctx) -> ActorRef`
...
- **Control plane (unbounded)**: `ActorBus` creates one unbounded `Event` inbox per actor. Both explicit actor-to-actor messages and child-to-owner notifications use the same addressable path. Non-blocking sends avoid cyclic backpressure deadlocks, and failed sends expose actors that have already exited.
- **Data plane (bounded)**: Packet queues for forwarding IP datagrams use `mpsc::channel::<Vec<PooledBuf>>(tuning.packet_queue_depth)` (default 256). Each channel message carries a batch of packets produced by a single device I/O operation (TUN `recv_batch`, BareUDP GRO, or up to 128 H3 datagrams / 64 KiB with adaptive flush on size change or small packets). This reduces per-packet atomic operations from N to 1 per batch. `packet_queue_depth` counts batch messages, not individual packets. Each batch contains up to `IDEAL_BATCH_SIZE` packets (Linux TUN), `gro_segments` chunks (BareUDP), or up to 128 datagrams (H3). Bounded channels provide backpressure, preventing memory exhaustion when producers outpace consumers. Buffers come from h3llo-owned static `buffer-pool` instances; dropped buffers return to their pool for reuse, reducing allocator pressure.

**Buffer allocation strategy**: Data-plane buffers reserve 10 bytes of headroom via `alloc_uninit_packet_buf(length)`. Requests where `length + HEADROOM ≤ MAX_DATAGRAM_SIZE` (1500) use a static pool of 1.5 KB buffers; larger requests use a static pool of 64 KB buffers. The allocator expands a reused `ConsumeBuffer` to the selected capacity and consumes the 10-byte prefix without zero-filling the payload. This remains necessary while quiche packet serialization and TUN receive accept only initialized slices rather than `BufMut` or an equivalent uninitialized-output API. The headroom supports both zero-copy CONNECT-IP prefix insertion and zero-copy `virtio_net_hdr` insertion. A compile-time assertion guards `HEADROOM >= VIRTIO_NET_HDR_LEN`. H3 datagram decoding removes its prefix with `pop_front`, restoring headroom for TUN TX. TUN RX and quiche DATAGRAM extraction fill uninitialized pooled output directly. UDP socket RX uses `alloc_packet_buf()` when splitting a shared GRO receive buffer into owned packets. The test-only tokio-quiche interoperability adapter converts between its `DgramBuffer` frames and production `PooledBuf`s at the adapter boundary.

**GSO batch sending**: The generic UDP TX actor (`udp::spawn_udp_tx`) concatenates packets from each `(SocketAddr, Vec<PooledBuf>)` batch into a contiguous buffer and sends via a single `sendmsg` with `segment_size` set (UDP GSO). Both BareUDP and H3v2 server transports share this actor. Batches are chunked by `max_gso_segments()` (typically 64 on Linux), which is refreshed after every send because quinn-udp can disable GSO after a driver error. On platforms without GSO support (`max_gso_segments() == 1`), the loop naturally degrades to per-packet sending. Any datagram transmission error returned by `UdpSocketState::try_send` drops the current destination batch without stopping the shared actor; failure to wait for socket writability still exits the actor and is handled by its spawn-time supervision policy. UDP actor drop metrics and rate-limited warnings remain future `ActorBus` work.
...
The orchestrator and control-plane actors (DNS, Route Sync, API) share the main thread's `current_thread` runtime. Cross-runtime communication uses Tokio MPSC channels, which are runtime-agnostic. `ActorBus` owns the runtime handles and task monitors, and every production actor supplies its diagnostic name, runtime, and exit policy at spawn time while inheriting its owner from `ActorContext`. Runtime-bound I/O resources are initialized through `ActorContext::run_on()` so callers never enter another runtime directly.

Orchestrator responsibilities and invariants:
- Maintain the latest configuration snapshot and H3 connection pool; receive control-plane events from other actors through its ActorBus inbox.
- Stay fully async: handle config updates, DNS refresh results, connection close notifications, and timer ticks without blocking other events.
- Spawn child actors (DNS resolver, H3 dialers, H3Dispatcher) through its `ActorContext` and register newly established H3 connections' TX channels in the Router's routing table via `Event::UpdateRouting`. `make_h3_dispatcher` prepares an `H3DispatcherGroup` containing the dispatcher and UDP actor states; `spawn_h3_dispatcher` assigns the shared listener actors a `Critical` policy so any component exit terminates the process instead of leaving a partial listener bundle alive.
...
- Handle graceful shutdown on `ctrl_c` signal; task exit errors include the actor name for debugging.
...
When configuration changes arrive (management API POST/DELETE or initialization), update the accepted-source filter first (fast, in-memory), then the internal routing table, then the system routing table. Dynamic reconfiguration flows through the orchestrator's ActorBus inbox.
...
- The DNS module owns the refresh timer internally; every `tuning.dns_refresh_interval` seconds, it queues A and AAAA queries for registered hostnames whose per-entry `next_refresh_at` timestamp has passed. When work is queued, `next_refresh_at` advances by the refresh interval, throttling redundant refresh triggers.
- A `HashSet<DnsQuery>` deduplicates queued work by hostname and record type. The query-work ticker sends at most one query per `tuning.dns_query_interval`, checks timeouts on the same cadence while pending work exists, and is disabled when both collections are empty. Both query-work and refresh tickers use `MissedTickBehavior::Delay` to prevent catch-up bursts after an idle period. Refresh and timeout handlers only add work to the set and therefore never sleep inside the actor's main `select` loop.
- An actor-wide `HashMap<DnsQuery, PendingQuery>` stores in-flight work. Each query-work tick scans the map and extracts every expired query directly into the retry set.
...
Route update summary: keep the TUN interface's routes aligned with `peers[].tun.allowed_ips` using `route_manager` APIs instead of shelling out (`ip route`, `route`, `netsh`). Route sync is performed by a dedicated Route Sync actor that serializes `Event::SyncRoutes` updates through its ActorBus inbox. The orchestrator sends updates without waiting for completion; the actor processes them one at a time in FIFO order. The `AsyncRouteManager` (netlink socket) is created once during actor initialization and reused across all sync operations. The TUN interface name is captured in the actor at spawn time (immutable for the orchestrator's lifetime). If the route manager cannot be initialized (e.g., BSD platforms without netlink), the orchestrator logs a warning and continues without system route management.

Route sync flow: (1) resolve the TUN ifindex, (2) list system routes, (3) drop stale TUN routes that are not covered by `allowed_ips` while preserving the configured TUN host addresses, (4) omit `allowed_ips` entries already covered by a configured TUN prefix and use exact matches for the remaining entries, (5) when `allowed_ips` includes a default route, split it into two `/1` entries (IPv4 and IPv6) once and warn about the split; if either `/1` conflicts with another interface, emit a conflict warning but still attempt to add the route without further splitting, and (6) warn on unsupported route entries or add/delete failures while continuing to run. Route sync failures are logged as warnings by the actor at origin.
...
Route deduplication: when synchronizing system routes, h3llo omits any desired route (from peer `allowed_ips`) already covered by a TUN address prefix (from `local.tun.addrs`). The connected TUN route is left for the OS to manage, avoiding redundant more-specific system routes.
...
- **DNS warnings logged at origin**: The DNS module logs warnings (NXDOMAIN, recursion refusal) directly via `warn!` instead of propagating them through events. Truncated (TC) responses are treated as packet loss: the pending entry stays in the map until `queue_timed_out_queries` queues a retry after `dns_query_timeout`; the query-work ticker performs the send. This follows the "log at origin" principle and simplifies event handling.
