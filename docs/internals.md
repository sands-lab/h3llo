...
3. Transport binding: for each resolved IP (HTTP/3) or selected outbound IP (BareUDP), probe the WAN-facing interface excluding the TUN; bind transport sockets to that interface. HTTP/3 creates connections across DNS answers and `peers[].h3.endpoints`, optionally bound to `peers[].h3.bindif` (auto-detected when unspecified); BareUDP uses a single listener socket (RX-only) and one outbound socket per BareUDP peer (TX-only). If probing or binding fails, warn and continue unbound.
...
- **Stateless DNS handling**: When a DNS answer arrives, the orchestrator iterates all peers to find those whose endpoint hostname matches the answer, then spawns TX actors for matching peers. This avoids maintaining a separate `pending_dns` state structure, keeping the orchestrator simpler and following DNS's inherently stateless nature.
