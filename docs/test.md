...
- Windows x64 CI: clean, uncached release build with a static MSVC C runtime; the resulting `h3llo.exe` is packaged for GitHub Releases.
- macOS ARM64 CI: clean, uncached native release build; the resulting Apple Silicon executable is checked for non-system dynamic dependencies and packaged for GitHub Releases.
