...
local:
  ...
  tun:
    ...
    addr:
    - 192.168.180.2/32
    mtu: 1280 # optional, default: 1280
peers:
...
- `local.bare.listen`: BareUDP listen address when using the plaintext fast path; optional alongside H3.
...
- `local.tun.mtu` (default `1280`): MTU for the TUN interface.
