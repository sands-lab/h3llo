...
- `local.h3.cert` / `local.h3.key`: Certificate chain and private key for QUIC/TLS; required when `local.h3` is set. Their parent directories are watched automatically. After a 500 ms quiet period, a matching pair is loaded transactionally and used for new connections; established connections keep their existing TLS sessions. A missing, unreadable, malformed, or mismatched pair is rejected while the previous valid configuration remains active.
...
- TLS rotation: Keep each credential's parent directory stable and rotate files or symlinks within it. Directory mounts are preferred in containers; Docker single-file bind mounts and Kubernetes `subPath` mounts do not reliably expose atomic file replacement. Native filesystem notifications may not arrive from NFS or other network filesystems, so use local storage or restart h3llo after rotation in those environments. `tuning.h3_trusted_ca` is client-side trust configuration and is not hot-reloaded.
