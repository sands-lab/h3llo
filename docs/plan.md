...
- [x] Step 12: Implement router actor: centralized userspace L3 forwarding in `src/router.rs` with LPM routing, TTL decrement (RFC 1624), batch splitting by destination; strip routing from `spawn_tun_rx`; rewire orchestrator so all RX actors send to router.
